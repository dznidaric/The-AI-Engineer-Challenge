...
6. **Important**: Set the `OPENAI_API_KEY` environment variable in Vercel:
...
> NOTE: The `vercel.json` is configured to serve the Next.js frontend as the main application and route `/api/*` requests to the FastAPI backend. The frontend will be accessible at the root URL, and the backend API will be available at `/api/chat`.
